...
crime.fit <- ergmm(crime_network ~ euclidean(d = 2, G = 3), verbose = TRUE)
...
network.vertex.names(crime_network)
...
# crime.fit2 <- ergmm(crime_network ~ euclidean(d=2, G=0), tofit=c("mle"))
...
#      labels = TRUE)
# title(sub = "Color represents the estimated groups: Labels the groups found by Sampson")
```
...
#  more printout.
crime.fit5 <- ergmm(crime_network ~ euclidean(d = 2, G = 5),
                      verbose = TRUE)
...
plot(crime.fit5, label = oneL, vertex.col = oneLcolors, what="mkl", main="mkl positions", print.formula=TRUE,
     labels = TRUE)
title(sub = "Color represents the estimated groups: Labels the groups found by Sampson")
...
require(ergm)
DSmod0 <- ergm(crime_network ~ edges,
